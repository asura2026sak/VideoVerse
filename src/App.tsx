...
import { Tv, Play, Radio, Users, Heart, Sparkles, Film, ArrowLeft, LayoutDashboard, Database, Menu, X, Search, ThumbsUp, Bookmark, Share2, Check } from "lucide-react";
...
export default function App() {
  ...
  // Open the effectivecpmnetwork smartlink in a new tab, honoring the shared
  // 15-minute cooldown cookie/localStorage used by the player (once per 15 min).
  const openSmartlinkInNewTab = () => {
    try {
      const hasAdCookie = document.cookie
        .split(";")
        .some((item) => item.trim().startsWith("vidverse_ad_played="));
      const adExpireVal = localStorage.getItem("vidverse_ad_expire");
      const isLSPermitted = adExpireVal
        ? parseInt(adExpireVal, 10) > Date.now()
        : false;

      if (hasAdCookie || isLSPermitted) {
        // Cooldown still active, skip opening
        return;
      }

      // Open via a real anchor click so browsers treat it as a normal new-tab link
      const a = document.createElement("a");
      a.href = "https://www.effectivecpmnetwork.com/hswm92uqx?key=910b8f90bb4e41f71cf53632f2325bcb";
      a.target = "_blank";
      ...
      console.warn("Failed to open smartlink new tab.", err);
    }
  };

  const handleSelectVideo = (video: Video) => {
    setActiveVideo(video);
    // Open the video in the fullscreen cinema modal player
    setIsVideoModalOpen(true);
    // Auto-load the smartlink in a new tab (respects the shared 15-min cooldown)
    openSmartlinkInNewTab();
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] font-sans text-slate-100 flex flex-col selection:bg-red-500/40 selection:text-white">
      
      {/* 2. Top Navigation Hub (YouTube-style navbar) */}
      <nav className="sticky top-0 z-50 bg-[#212121] border-b border-[#303030] px-3 md:px-4 py-2.5">
        <div className="max-w-[1920px] mx-auto flex items-center justify-between gap-3">
          {/* Left: Mobile hamburger + Logo */}
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className="flex lg:hidden p-2 rounded-full text-slate-300 hover:text-white hover:bg-white/10 transition-all cursor-pointer focus:outline-none"
            ...
            <div className="flex items-center gap-2 cursor-pointer select-none" onClick={() => navigateTo("/")}>
              <div className="w-8 h-8 rounded-md bg-[#ff0000] flex items-center justify-center shadow-md shadow-red-500/30">
              ...
              <div className="hidden sm:flex flex-col">
                <span className="font-display text-xl font-black tracking-tight text-white leading-none">
                  Video<span className="text-[#ff0000]">Verse</span>
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 max-w-2xl mx-auto items-center justify-center">
            <form
              ...
              className="flex w-full max-w-xl items-center rounded-full border border-[#3f3f3f] bg-[#121212] focus-within:border-sky-500/60 overflow-hidden transition-colors"
            >
              <input
                type="text"
                value={searchQuery}
                ...
                placeholder="Search"
                className="flex-1 bg-transparent px-4 py-2 text-sm text-white placeholder:text-slate-500 outline-none"
              />
              <button
                type="submit"
                className="px-4 py-2 bg-[#272727] border-l border-[#3f3f3f] text-slate-300 hover:text-white transition-colors cursor-pointer"
                aria-label="Search"
              >
          ...
          {/* Right: Navigation links + Indicators */}
          <div className="flex items-center gap-2 lg:gap-3 text-xs text-slate-400 font-medium shrink-0">
            <div className="hidden lg:flex items-center gap-1 col-nav">
              <button
                onClick={() => navigateTo("/")}
                className={`px-3 py-1.5 rounded-full text-xs font-bold font-mono tracking-wider uppercase transition-all duration-150 cursor-pointer ${
            ...
            <button
              ...
              className="hidden md:flex items-center gap-1.5 px-3 py-2 bg-[#272727] hover:bg-white/10 border border-[#3f3f3f] text-slate-300 hover:text-white rounded-full text-[10px] font-mono transition-all cursor-pointer font-bold"
            >
              <LayoutDashboard className="w-3.5 h-3.5 text-red-500" />
              <span>{isAdminView ? "USER FEED" : "ADMIN"}</span>
            </button>
          </div>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-[#212121] border-b border-[#303030] text-slate-200 overflow-hidden sticky top-[57px] z-45"
          >
            <div className="px-4 py-5 flex flex-col gap-4">
              <div className="text-[9px] uppercase text-red-500 font-extrabold font-mono tracking-wider">
              ...
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-semibold font-mono transition-all border ${
                  (!isAdminView && !isAboutView) 
                    ? "bg-slate-900/80 text-red-400 border-red-500/20" 
              ...
              >
                <span className="w-4 h-4 text-red-400 flex items-center justify-center text-xs">📝</span>
              ...
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-semibold font-mono transition-all border ${
                  isAdminView 
                    ? "bg-slate-950 text-amber-405 border-amber-500/20" 
                    : "text-slate-300 hover:bg-slate-900/50 border-transparent"
                }`}
              >
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between px-3.5 py-2.5 bg-slate-900/40 rounded-xl text-xs font-mono border border-slate-900/50">
                  ...
                  <span className="text-red-400 font-bold">{totalLikesCalculated}</span>
                </div>
                ...
                <div className="flex items-center justify-between px-3.5 py-2.5 bg-slate-900/40 rounded-xl text-xs font-mono border border-slate-900/50">
                  ...
                  <span className="text-red-400 font-bold">{watchlistCount}</span>
                </div>
      ...
      <AnimatePresence mode="wait">
        {isAdminView ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-5 flex flex-col gap-6">
                ...
                <div className="p-6 bg-slate-900/60 border border-slate-850 rounded-2xl flex flex-col gap-3 font-mono text-xs">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-red-400 font-bold">{videos.length} Streams</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="p-6 bg-slate-900/40 border border-slate-850/60 rounded-2xl flex flex-col gap-3 text-xs leading-relaxed">
                  ...
                  <p className="text-slate-400">
                    The backend writes the file to the physical <code className="bg-slate-950 px-1 py-0.5 rounded text-red-400">/uploads</code> folder on disk where it becomes immediately accessible over http static files routing, and automatically publishes the new feed.
                  </p>
                </div>
          ...
          >
            ...
            <div className="bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 border border-slate-800 p-8 md:p-12 rounded-3xl relative overflow-hidden shadow-2xl">
              ...
              <div className="relative z-10 max-w-3xl">
                <span className="text-[10px] bg-red-400/10 border border-red-500/20 text-red-400 font-mono uppercase tracking-widest font-bold px-3 py-1 rounded-md">
                  Who We Are
                </span>
                <h1 className="text-3xl md:text-5xl font-extrabold text-white mt-4 tracking-tight leading-tight">
                  Next-Generation <br />
                  <span className="bg-gradient-to-r from-red-400 via-amber-300 to-amber-300 bg-clip-text text-transparent">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="about-detailed-descriptions">
              ...
              <div className="bg-slate-900/40 border border-slate-850 p-6 md:p-8 rounded-2xl flex flex-col gap-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-red-500/10 to-amber-500/10 border border-red-500/20 flex items-center justify-center text-red-400">
                ...
                <div className="mt-4 pt-4 border-t border-slate-800/60 flex flex-col gap-2 font-mono text-[11px] text-slate-500">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-red-400">Low-Latency Static CDN</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900/40 border border-slate-850 p-6 md:p-8 rounded-2xl flex flex-col gap-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500/10 to-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400">
                ...
                <button
                  ...
                  className="mt-auto px-4 py-2.5 bg-gradient-to-r from-red-500/10 to-amber-500/10 hover:from-red-500/20 hover:to-amber-500/20 border border-red-500/20 hover:border-red-500/40 text-red-400 text-xs font-mono font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                >
          ...
          >
            ...
            <div className="flex flex-col gap-10 order-2 w-full animate-fadeIn">
              
              {/* Bottom Section: Full Width Stream Listing */}
            ...
            <div className="bg-slate-900/30 border border-slate-900/80 p-6 md:p-8 rounded-2xl flex flex-col md:flex-row gap-6 justify-between items-start md:items-center mt-4 order-3">
              <div className="max-w-4xl">
                <span id="about-label" className="text-[10px] bg-red-400/10 border border-red-500/20 text-red-400 font-mono uppercase tracking-widest font-bold px-2.5 py-1 rounded-md">
                  About Us
                </span>
              ...
              <div className="bg-slate-950/60 border border-slate-850 p-4 rounded-xl max-w-xs w-full text-xs font-mono text-slate-400 flex flex-col gap-1.5 shrink-0">
                <div className="text-slate-350 font-bold border-b border-slate-800 pb-1 uppercase tracking-wider text-[10px] flex items-center gap-1.5 text-red-400">
      ...
      <footer className="border-t border-[#303030] bg-[#0f0f0f] py-8 px-4 md:px-6 lg:px-8 xl:px-12 mt-16">
        <div className="max-w-[1920px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-slate-500 font-medium">
          ...
          <div className="flex items-center gap-4 text-slate-400">
            <span className="hover:text-red-450 transition-colors cursor-pointer">Terms of Service</span>
            <span>•</span>
            <span className="hover:text-red-450 transition-colors cursor-pointer">Privacy Policy</span>
            <span>•</span>
            <span className="hover:text-red-450 transition-colors cursor-pointer">VOD Solutions</span>
          </div>
        </div>
      </footer>
        ...
        {isVideoModalOpen && (
          ...
          >
            ...
            >
              ...
              {/* Action toolbar overlaid on the modal player */}
              <div className="absolute top-4 left-4 z-40 flex items-center gap-2">
                <button
                  onClick={() => handleToggleLike(activeVideo.id)}
                  className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-semibold border transition-all duration-200 active:scale-95 backdrop-blur-md cursor-pointer ${
                    likedVideoIds.includes(activeVideo.id)
                      ? "bg-[#ff0000] text-white border-[#ff0000] shadow-lg shadow-red-500/40"
                      : "bg-black/60 text-slate-200 border-white/15 hover:bg-black/80 hover:text-white"
                  }`}
                  title="Like this video"
                >
                  <ThumbsUp className="w-4 h-4" />
                  <span>{activeVideo.likes + (likedVideoIds.includes(activeVideo.id) ? 1 : 0)}</span>
                </button>
                ...
                >
                  <Bookmark className={`w-4 h-4 ${bookmarkedVideoIds.includes(activeVideo.id) ? "fill-current" : ""}`} />
                  <span>{bookmarkedVideoIds.includes(activeVideo.id) ? "Saved" : "Watch Later"}</span>
                </button>

                <button
                  onClick={handleShareVideo}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-semibold bg-black/60 text-slate-200 border border-white/15 hover:bg-black/80 hover:text-white backdrop-blur-md transition-all duration-200 active:scale-95 cursor-pointer"
                  title="Copy share link"
                >
                  {isShareCopied ? <Check className="w-4 h-4 text-emerald-500" /> : <Share2 className="w-4 h-4" />}
